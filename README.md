...
审阅选项卡的「本地历史」：有改动时每 5 分钟自动存一份工程快照（可关），也可以随时手动存；选一份看它跟现在的差异（按正文、摘要、结论、元信息、设置、文献等逐段对照），能整份恢复，恢复前会先存一份「恢复前」。快照 gzip 后存在浏览器的 IndexedDB 里，每个工程最多留 80 份，先淘汰最老的自动快照。图片不进快照。

### Git

审阅选项卡的「Git」：启用后每次提交把 Typst 源（`main.typ`、`refs.bib`、`images/…`）和整份工程（`project.iota.json`）记下来，能看每次提交改了哪些文件、逐行差异，能恢复到某次提交。连上 GitHub（fine-grained 个人访问令牌，只选这个仓库、给 Contents 读写权限；令牌只存在当前浏览器会话里）后可以推送、拉取：推送把本地没推过的提交按顺序造到远端分支，远端有别处的新提交就先拉；拉取在工作区干净时取回远端整份工程与图片，本地记一条镜像提交。不分支、不暂存。
