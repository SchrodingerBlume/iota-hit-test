...
- 右侧实时预览（typst.ts 渲染成 SVG）：停手 0.13 秒就重排；编译走增量服务，渲染只补丁变了的页（按 data-tid 复用没变的 `<g>`），十几页的文档主线程每次只花几毫秒；四十页以上打字时只编光标所在的那一章（另开一份 `focus.typ` 与增量服务，章前后的页照旧，文献表那几页纸宽标成 500pt 由渲染端剔掉），停手 2.5 s 再整编一次校准页码与目录——177 页的论文每个字 0.2～0.3 s，左侧编辑与预览里直接编辑都走这条，那一章自己的字形表并进整编那份（章外的位置换算到新版本）；再往下一层是打字即时回显：光标所在是纯文字段（只有文字与字符级格式）就先只编这一段（`/para.typ`，85 ms），单独画成一小张 SVG 贴回它原来的位置、原来的行用纸色盖住，字形表也盖进去，光标立刻跟上——键入到看见约 150～200 ms，段撑过页底那一截先画在页外、等章级结果落地归位；重排落地时挪了位置的文字块从旧位置滑过去、新块淡入；导出 PDF 一键
...
线性内存只涨不缩，过 3.6 GB 趁空闲预防性换一个 worker（字体、图片重发；字体没回来前不编，不然会先用替代字体排一遍再重排），真陷进去（unreachable / recursive use）也自动换。
...
段首 `#`～`####` 加空格成一到四级标题，`- ` / `* ` 无序列表，`1. ` 编号列表，`> ` 引用，```` ``` ```` 代码块；行内 `**粗**`、`*斜*`、
`` `等宽` ``、`~~删除~~`；`$x^2$` 敲完后面那个 `$` 就成行内公式，空段里 `$$` 加空格成一条行间公式并展开编辑（`$5 和 $6` 这种带空格的不会误判）。
