  ...
  return (
    <NodeViewWrapper className={`blk alg ${selected ? 'is-selected' : ''}`} ref={wrap} data-drag-handle>
      <div className="alg-head" contentEditable={false}>
        {num && <span className="cap-num" title="编号按模板规则算，预览为准">{num}</span>}
        <AutoInput className="cap-input" data-attr="caption" disabled={!editable} value={node.attrs.caption ?? ''} placeholder="算法题" minWidth={80} onChange={(e) => updateAttributes({ caption: e.target.value })} />
        <AutoInput className="cap-input-en" data-attr="captionEn" disabled={!editable} value={node.attrs.captionEn ?? ''} placeholder="English caption（可空）" minWidth={60} onChange={(e) => updateAttributes({ captionEn: e.target.value })} />
      </div>
      <div className="alg-body" contentEditable={false}>
        ...
        {lines.map((l, i) => (
          <div key={i} className="alg-line" style={{ paddingLeft: `${l.level * 1.6}em` }}>
            ...
            <input value={l.text} disabled={!editable} placeholder={i === 0 ? 'for $t = 1$ to $T$ do   （Tab 缩进，回车加一行）' : ''} onChange={(e) => setLines(lines.map((x, k) => (k === i ? { ...x, text: e.target.value } : x)))} onKeyDown={(e) => onKey(i, e)} />
            <span className="alg-tools">
              <button type="button" className="alg-btn" title="减少缩进（Shift+Tab）" disabled={!editable || !l.level} onClick={() => setLines(lines.map((x, k) => (k === i ? { ...x, level: Math.max(0, x.level - 1) } : x)))}><ChevronLeft /></button>
              <button type="button" className="alg-btn" title="增加缩进（Tab）" disabled={!editable} onClick={() => setLines(lines.map((x, k) => (k === i ? { ...x, level: Math.min(6, x.level + 1) } : x)))}><ChevronRight /></button>
              <button type="button" className="alg-btn" title="上移" disabled={!editable || i === 0} onClick={() => move(i, -1)}><ArrowUp /></button>
              <button type="button" className="alg-btn" title="下移" disabled={!editable || i === lines.length - 1} onClick={() => move(i, 1)}><ArrowDown /></button>
              <button type="button" className="alg-btn" title="删掉这行" disabled={!editable || lines.length <= 1} onClick={() => setLines(lines.filter((_, k) => k !== i))}><Trash2 /></button>
            </span>
          </div>
        ))}
      </div>
      <div className="blk-tools" contentEditable={false} onMouseDown={(e) => e.stopPropagation()}>
        <button type="button" className="blk-tool is-btn" disabled={!editable} onClick={() => setIo([...io, io.length ? 'output: ' : 'input: '])}><Plus />输入 / 输出行</button>
        <button type="button" className="blk-tool is-btn" disabled={!editable} onClick={() => { setLines([...lines, { text: '', level: 0 }]); focusLine(lines.length); }}><Plus />加一行</button>
        ...
        <button type="button" className="blk-tool is-btn is-danger" title="删除算法" disabled={!editable} onClick={deleteNode}><Trash2 /></button>
      </div>
    </NodeViewWrapper>
  );
}
