...
const LEADS: { value: string; label: string; hint: string }[] = [
  { value: 'auto', label: '式中', hint: '引导词按文档语言取：中文「式中」、英文 where' },
  { value: 'none', label: '不印', hint: '手动拆成两段时后半段用：不重复引导词，左缘照旧对齐' },
];
...
function SymbolCell({ row, onChange, editable }: { row: DenoteRow; onChange: (r: Partial<DenoteRow>) => void; editable: boolean }) {
  ...
  return (
    <span className="denote-pop" ref={ref}>
      <button type="button" className="denote-sym-btn" disabled={!editable} title="点击编辑符号（LaTeX 或 Typst）" onMouseDown={(e) => e.preventDefault()} onClick={() => setOpen((o) => !o)}>
        {parts.length ? parts.map((p, i) => <span key={i}>{i > 0 && '、'}<MathPreview src={forPreview(p, mode)} mode={mode} /></span>) : <em>符号</em>}
      </button>
      {open && (
        <span className="chip-pop chip-pop-wide" onMouseDown={(e) => e.stopPropagation()}>
          ...
          <div className="muted" style={{ fontSize: 11, marginTop: 4 }}>一条挂多个符号用「、」隔开，如 <code>R_t、\omega_t</code></div>
        </span>
      )}
    </span>
  );
...
function EqDenoteView({ node, updateAttributes, selected, deleteNode, editor, getPos }: NodeViewProps) {
  ...
  const leadText = lead === 'auto' ? '式中' : lead === 'none' ? '' : lead;
  return (
      ...
      <div className="denote-grid" contentEditable={false}>
        {rows.map((r, i) => (
          <div key={i} style={{ display: 'contents' }}>
            <span className={`denote-lead ${i === 0 && !leadText ? 'is-muted' : ''}`}>{i === 0 ? (leadText || '（不印引导词）') : ''}</span>
            ...
            <span className="denote-meaning">
              <input data-attr={`rows.${i}.meaning`} value={r.meaning} placeholder="物理量的名称与单位，如：多孔质材料的平均粒子直径（m）" disabled={!editable}
            ...
            <button type="button" className="blk-tool is-btn is-danger denote-del" title="删掉这一条" disabled={!editable} onClick={() => remove(i)}><X /></button>
          </div>
        ))}
      </div>
      <div className="denote-foot" contentEditable={false}>
        <button type="button" className="btn btn-xs" disabled={!editable} onClick={() => add(rows.length - 1)}><Plus />加一条</button>
        <span className="seg" title="引导词">
        ...
        <span className="muted">规范 2.11：破折号后第一个字对齐，转行悬挂；一条挂多个符号写「x、y」</span>
        <span className="spacer" />
        <button type="button" className="blk-tool is-btn is-danger" title="删除整块" disabled={!editable} onClick={deleteNode}><Trash2 /></button>
      </div>
    </NodeViewWrapper>
  );
}
