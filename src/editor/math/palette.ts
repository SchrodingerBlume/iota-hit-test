...
export const PALETTE: PaletteGroup[] = [
  {
    ...
    items: [
      ...
      S('x_{i}', '□_(□)', '□_{□}', '下标'),
      S('x_{i}^{2}', '□_(□)^(□)', '□_{□}^{□}', '上下标'),
      S('\\sqrt{x}', 'sqrt(□)', '\\sqrt{□}', '平方根'),
      S('\\sqrt[n]{x}', 'root(□, □)', '\\sqrt[□]{□}', 'n 次根'),
      S('\\sum_{i=1}^{n}', 'sum_(□)^(□)', '\\sum_{□}^{□}', '求和'),
      S('\\prod_{i=1}^{n}', 'product_(□)^(□)', '\\prod_{□}^{□}', '连乘'),
      S('\\int_{a}^{b}', 'integral_(□)^(□)', '\\int_{□}^{□}', '积分'),
      S('\\iint', 'integral.double', '\\iint', '二重积分'),
      S('\\oint', 'integral.cont', '\\oint', '环路积分'),
      S('\\lim_{x\\to 0}', 'lim_(□ -> □)', '\\lim_{□ \\to □}', '极限'),
      S('\\frac{\\mathrm{d}y}{\\mathrm{d}x}', '(dif □)/(dif □)', '\\frac{\\mathrm{d}□}{\\mathrm{d}□}', '导数'),
      S('\\frac{\\partial y}{\\partial x}', '(diff □)/(diff □)', '\\frac{\\partial □}{\\partial □}', '偏导'),
      S('\\left(x\\right)', '(□)', '\\left(□\\right)', '括号'),
      S('\\left[x\\right]', '[□]', '\\left[□\\right]', '方括号'),
      S('\\left\\{x\\right\\}', '{□}', '\\left\\{□\\right\\}', '花括号'),
      S('\\left|x\\right|', 'abs(□)', '\\left|□\\right|', '绝对值'),
      S('\\left\\|x\\right\\|', 'norm(□)', '\\left\\|□\\right\\|', '范数'),
      S('\\langle x\\rangle', 'angle.l □ angle.r', '\\langle □ \\rangle', '尖括号'),
      S('\\lfloor x\\rfloor', 'floor(□)', '\\lfloor □ \\rfloor', '向下取整'),
      S('\\lceil x\\rceil', 'ceil(□)', '\\lceil □ \\rceil', '向上取整'),
      S('\\binom{n}{k}', 'binom(□, □)', '\\binom{□}{□}', '二项式'),
      S('\\mathrm{e}^{x}', 'e^(□)', '\\mathrm{e}^{□}', '指数'),
    ],
  },
  {
    name: '矩阵',
    items: [
      S('\\begin{pmatrix}a&b\\\\c&d\\end{pmatrix}', 'mat(□, □; □, □)', '\\begin{pmatrix} □ & □ \\\\ □ & □ \\end{pmatrix}', '2×2 矩阵（圆括号）'),
      S('\\begin{bmatrix}a&b\\\\c&d\\end{bmatrix}', 'mat(delim: "[", □, □; □, □)', '\\begin{bmatrix} □ & □ \\\\ □ & □ \\end{bmatrix}', '2×2 矩阵（方括号）'),
      S('\\begin{vmatrix}a&b\\\\c&d\\end{vmatrix}', 'mat(delim: "|", □, □; □, □)', '\\begin{vmatrix} □ & □ \\\\ □ & □ \\end{vmatrix}', '行列式'),
      S('\\begin{pmatrix}a&b&c\\\\d&e&f\\\\g&h&i\\end{pmatrix}', 'mat(□, □, □; □, □, □; □, □, □)', '\\begin{pmatrix} □ & □ & □ \\\\ □ & □ & □ \\\\ □ & □ & □ \\end{pmatrix}', '3×3 矩阵'),
      S('\\begin{pmatrix}a\\\\b\\end{pmatrix}', 'vec(□, □)', '\\begin{pmatrix} □ \\\\ □ \\end{pmatrix}', '列向量'),
      S('\\begin{cases}a&x>0\\\\b&x\\le 0\\end{cases}', 'cases(□ & "if" □, □ & "otherwise")', '\\begin{cases} □ & □ \\\\ □ & □ \\end{cases}', '分段函数'),
  ...
  },
  {
    ...
    items: [
      ...
      S('\\mathbf{x}', 'bold(□)', '\\mathbf{□}', '粗体'), S('\\mathrm{x}', 'upright(□)', '\\mathrm{□}', '正体'), S('\\mathbb{R}', 'RR', '\\mathbb{R}', '实数集'), S('\\mathcal{L}', 'cal(□)', '\\mathcal{□}', '花体'), S('\\text{文字}', '"□"', '\\text{□}', '文字'),
