...
import {
  ...
  Lightbulb20Regular, CloudArrowDown20Regular, ZoomIn20Regular, AutoFitWidth20Regular, DocumentOnePage20Regular, DocumentMultiple20Regular, TextChangeCase20Regular, TextWordCount20Regular, PanelLeftText20Regular, ChevronDoubleRight16Regular, DocumentHeader20Regular, DocumentFooter20Regular,
...
export function Ribbon({ layout, leading, trailing, minimal }: { layout: RibbonLayout; leading?: ReactNode; trailing?: ReactNode; minimal?: boolean }) {
  ...
  useEffect(() => startAutoHistory(() => (useStore.getState().view === 'editor' ? useStore.getState().doc : null), (cb) => useStore.subscribe((s, prev) => { if (s.doc !== prev.doc && s.view === 'editor') cb(); })), []);
  ...
  return (
    <div ref={root} className={`ribbon ${none ? 'is-idle' : ''} ${collapsed ? 'is-collapsed' : ''} ${peek ? 'is-peek' : ''}`} onClick={(e) => { const t = e.target as HTMLElement; if (t.closest('.rb-btn') && !t.closest('.rb-keep')) afterCommand(); }}>
      ...
      <HistoryDialog />
      <GitDialog />
      <ZoteroDialog open={zoteroOpen} onClose={() => setZoteroOpen(false)} entries={references} onChange={(e) => useStore.getState().setReferences(e)} />
