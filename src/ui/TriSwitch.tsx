    ...
    <div className="triseg">
      ...
      <div className="seg triseg-seg" role="radiogroup" aria-label={label}>
        <button type="button" role="radio" aria-checked={isAuto} className={`auto ${isAuto ? 'on' : ''}`} onClick={() => onChange('auto')} title={`自动：${auto.reason}`}>
          自动<span className="triseg-arrow">：</span><b className={`t-${toneOf(auto.value)}`}>{labelOf(auto.value)}</b>
        </button>
        {choices.map((c) => (
      ...
      {showAutoWhenFixed && !isAuto && auto.value !== value && <div className="triseg-note">默认：{labelOf(auto.value)}</div>}
    </div>
  );
}

export const ON_OFF: SegChoice<boolean>[] = [
  { value: false, label: '否', tone: 'off' },
  { value: true, label: '是', tone: 'on' },
];
